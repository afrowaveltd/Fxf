...
/// <summary>
/// SignalR client contract for localization-related notifications and requests. Implement this
/// interface on the client to receive updates about locales, language metadata, dictionaries, and
...
public interface ILocalizationInterface
{
	...
	/// </summary>
	/// <remarks>
	/// This method is typically used to identify the locales that are supported by the application
	/// and perform any necessary initialization or configuration based on the detected locales.
	/// </remarks>
	...
	Task OnDictionariesRefreshed(string scope);
	...
	/// </summary>
	/// <param name="isClient">
	/// True to target client (WebAssembly) locales; false for server locales. Defaults to true.
	/// </param>
	...
	/// <param name="result">
	/// A response object that will be populated with the translation result. The caller is
	/// responsible for handling the response's success or failure state.
	/// </param>
	/// <returns>A task that represents the asynchronous operation.</returns>
	...
	/// </summary>
	/// <param name="isClient">
	/// True when the data is for client (WebAssembly) context; false for server context.
	/// </param>
	...
	/// </summary>
	/// <param name="isClient">
	/// True if the operation targeted client (WebAssembly) data; false for server.
	/// </param>
