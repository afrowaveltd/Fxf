...
/// <summary>
/// Provides language and localization utilities for the application, including:
/// language metadata, discovery of available locale files, loading and saving
/// translation dictionaries, and helper operations.
/// </summary>
public class LanguageService(IConfiguration configuration, IStringLocalizer<LanguageService> t) : ILanguageService
{
	...
	/// </summary>
	/// <param name="isFrontend">When true, checks the client (WebAssembly) locales directory; otherwise the server locales directory.</param>
	...
	/// Unknown codes are returned as placeholder <see cref="Language"/> objects that echo the code as name/native.</returns>
	...
	/// <returns>A <see cref="Response{T}"/> with the key/value dictionary on success. Returns a warning when the file is empty.</returns>
	...
	/// <returns>A <see cref="Response{T}"/> with the last stored dictionary when available; otherwise a failed response.</returns>
	...
	/// </summary>
	/// <param name="isFrontend">When true, reads from the client (WebAssembly) locales folder; otherwise from the server locales folder.</param>
	/// <returns>A <see cref="Response{TranslationTree}"/> containing translations mapped per language.</returns>
...
#pragma warning restore CS1584 // XML comment has a syntactically incorrect cref attribute.
	...
	/// <param name="isFrontend">When true, saves to the client (WebAssembly) locales folder; otherwise to the server locales folder.</param>
	/// <returns>A <see cref="Response{T}"/> with a per-language success map (key = language code, value = success indicator).</returns>
